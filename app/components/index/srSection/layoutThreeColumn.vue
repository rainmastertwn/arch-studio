...
const isQ6Data = computed(() => {
  return props.postData?.value === 'Q₆透滲透排水管'
})
</script>

<template>
  <div class="flex flex-col justify-center gap-5 lg:flex-row items-start"">
      ...
      <div>
        ...
        <ul class="mb-3">
          <li
            ...
            :key="index"
          >
            <span class="block min-w-[30px] text-base" v-if="item.label">{{ item.label }}</span>
            <span
              class="font-light"
              v-html="item.content"
              :class="{
    ...
    <div class="flex basis-1/1 flex-col justify-between lg:basis-1/3 min-h-[450px]">
      ...
      <div class="overflow-hidden rounded-2xl" v-if="postData?.infoImg[1]">
        <img class="mx-auto h-full w-full" :src="postData?.infoImg[1]?.imgSrc" />
      </div>
    </div>
    ...
    <div class="basis-1/1 lg:basis-1/3">
      ...
      <div class="mb-5" v-if="!isQ6Data && postData?.info[3]">
        ...
        <ul class="mb-3">
          <li
            class="mb-2 flex text-lg"
            v-for="(item, index) in postData?.info[3]?.items"
            :key="index"
          >
            <span class="block min-w-[30px] text-base" v-if="item.label">{{ item.label }}</span>
            <span
